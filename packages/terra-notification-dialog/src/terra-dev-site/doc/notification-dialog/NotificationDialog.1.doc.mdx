...
The Notification Dialog presentations are grouped into categories of risk vs. non-risk, and prioritized by urgency of a required response. They include fixed icons, colors, and ANSI signal words to provide consistency in risk mitigation required by regulatory affairs and safety certifications.

- Alert, Warning, and Information/Advisory are notifications that are intended to make users aware of risk or hazard, such as risk to patient safety or financial risk, and have three levels of severity: `hazard-high`, `hazard-medium` and `hazard-low`.

- Other situations may require notifications that inform a user to system activity and required actions, but not classified as risks. A provided dialog for a common non-hazard is the `error` variant. 

- The `custom` variant allows for less common standardized non-hazard situations, requiring an icon and signal word be provided in addition to the common props. By choosing to create a custom Notification Dialog, the consumer is responsible for providing an icon that is themed correctly for the terra-default-theme, orion-fusion-theme, and clinical-lowlight-theme. See Terra's [Theme Strategy Guide](/guides/terra-application/theme-strategy) for more info.
...
## Notification Dialogs for Hazards & Risks

<HazardHighVariant
  title="Alert Notification Dialog (Hazard-High)"
/>
<HazardMediumVariant
  title="Warning Notification Dialog (Hazard-Medium)"
/>
<HazardLowVariant
  title="Information/Advisory Notification Dialog (Hazard-Low)"
/>

## Notification Dialogs for Non-Hazards

<ErrorVariant
  title="Error Notification Dialog (Non-Hazard High)"
/>
<CustomVariant
  title="Create a custom Notification Dialog (Non-Hazard Medium)"
/>
